...
const mobileTitle = read('mobile-title.js');
...
assert.match(html, /mobileAnimation\.src = '\.\/mobile-title\.js'/);
assert.match(html, /<canvas id="mobile-title" width="256" height="113"><\/canvas>/);
assert.doesNotMatch(mobileTitle, /^\s*import\s/m);
assert.doesNotMatch(mobileTitle, /new CRT|\.render\(|getContext\(['"]webgl/);
assert.match(mobileTitle, /last: 32, ms: 90, pause: 2000, rest: 150/);
assert.match(mobileTitle, /prefers-reduced-motion: reduce/);
assert.match(mobileTitle, /title\.fetchPriority = 'low'/);
...
assert.doesNotMatch(html, /@keyframes entry-scan|animation: entry-scan/);
