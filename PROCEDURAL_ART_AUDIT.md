...
7d. **MONUMENT SERIES — a tiered legend-monument set reflecting battle magnitude (owner-approved 2026-07-23).** *(art series + a small determinism-safe sim tag)* — Extends the reworked `makeMonument`. Five tiers, escalating with the gravity of the felling: **1 Cairn** (rough stone pile — minor felling, no losses) · **2 Marker Stone** (the current obelisk+plaque — a clean stand) · **3 Sworded Stele** (taller stone + planted blade + laurel — defenders wounded) · **4 Cenotaph** (stepped/twin memorial + broken shield/helm + scorched earth — heavy losses, stores torn away) · **5 War Barrow** (grandest — marker per life lost, shattered crossed weapon, blackened ground — catastrophe). **Tier = the HIGHEST of three signals**: foe difficulty (common → repeat nemesis), lives lost (wounded → downed → killed defenders), destruction (stores carried / raider-band size). Pieces: (a) `farm.js` — stamp `m.tier` at monument creation from the ALREADY-seeded raid outcome (no `world.rand`; monuments aren't in the determinism digest, so baselines hold; old saves default tier 2); (b) `pixel.js` — `makeMonument(tier)` → 5 cached STONE-ramp sprites with loss/destruction symbols scaling up; (c) `main.js` — render + hover pick by `m.tier`. Separate from the single town guardian statue (which already tier-upgrades). Queued AFTER the animation pass (7c).

7c. **ANIMATION PASS — bring the static procedural set to life.** *(display-time, `performance.now`-driven, deterministic — NO sim/seed reads; each must read at 1×)* — **Motion is a first-class quality lever** (owner calibration 2026-07-23: the animated windmill is THE exemplar — "EXACTLY what I'm looking for… especially due to its use of animation"). Only the windmill + koi/fish move today; the rest sit still. Targets: tower amber-orb **pulse**; hatchery + mill chimney **smoke** (reuse the existing house-smoke system); pond **ripple/shimmer** + a lily **bob**; a gentle crop-field **sway**; a barn/coop **weathervane or flag**. A spinning/pulsing silhouette also survives 1× better than fine detail. Also **bake motion into the orc variants from the start** (iron-bladed twisted windmill, sickly pulsing orb, forge-smoke).
